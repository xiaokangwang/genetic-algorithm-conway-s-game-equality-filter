...
func (sc *society) CheckPoint() {
	...
	err := os.MkdirAll(detailfoldername, 0777)
	...
	file, err := os.Create(detailfoldername + "socity.json")
	fmt.Println(debugTraceStart)
	fmt.Println("Checkpoint cp")
	if err != nil {
	...
	err = os.Symlink("../../"+detailfoldername+"/socity.json", currentSymname)
	fmt.Println(debugTraceStart)
	fmt.Println("Checkpoint cp")
	if err != nil {
...
func (sc *society) Restore(seedname string) {
	...
	file, err := os.Open(currentSymname)
	fmt.Println(debugTraceStart)
